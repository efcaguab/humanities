...
It is much easier to manage the data if the data frame is "tall" from the begining. This is because that way it's not necessary to enter any name or row number manually. 

To have a tidy dataframe from the beginning we leverage the power of the function `na.locf` from the `zoo` package. This function fills NA values (that arrise from the merged cells in Excel) with the last non-na cell before. First we use it across rows to create consistent column names, and then across cols to fill the field of study.
...
```{r tall data}
# It's all done :) no need to anything manually, just need to rename the columns so they match the names you gave them
